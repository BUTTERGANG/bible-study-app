...
import { BookOpen, ChevronLeft, ChevronRight, Check, Library, Search, Sparkles, X } from 'lucide-react'
...
const SUMMARY_LENGTHS = [
  { id: 'brief', label: 'Brief', desc: '3-5 bullets' },
  { id: 'standard', label: 'Standard', desc: 'paragraph + outline' },
  { id: 'detailed', label: 'Detailed', desc: 'full sectioned analysis' },
]
...
export default function LibraryReader() {
  ...
  function handleSummarize() {
    ...
    abortRef.current = streamSummarize(
      { resource_id: selectedBook.id, summary_length: summaryLength },
      ({ stage, text, cached }) => {
        if (stage === 'status') {
          ...
          setSummaryDone(true)
          setSummaryLoading(false)
          if (cached) setSummaryCached(true)
        }
      },
      (err) => {
        setSummaryLoading(false)
  ...
  async function handleSaveToNotes() {
    if (!selectedBook || noteSaving || noteSaved) return
    setNoteSaving(true)
    const lengthLabel = SUMMARY_LENGTHS.find((l) => l.id === summaryLength)?.label ?? 'Standard'
    const lines = []
    lines.push(`# Summary of "${selectedBook.title}"`)
    if (selectedBook.author) lines.push(`*by ${selectedBook.author}*`)
    lines.push(`*${lengthLabel} summary — generated by AI*`)
    lines.push('')
    if (summaryTldr) {
      lines.push('## TL;DR')
      lines.push(summaryTldr)
      lines.push('')
    }
    if (summaryKeyPoints.length > 0) {
      lines.push('## Key Points')
      summaryKeyPoints.forEach((pt) => lines.push(`- ${pt}`))
      lines.push('')
    }
    if (summaryOutline) {
      lines.push('## Outline')
      lines.push(summaryOutline)
    }
    try {
      await api.createNote({
        ...
        chapter: 0,
        content: lines.join('\n'),
        tags: `library-summary,${selectedBook.category || 'book'}`,
      })
      setNoteSaved(true)
    } catch {
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="px-4 pt-3 pb-2 border-b border-gray-100 dark:border-gray-700">
              ...
              <div className="flex gap-1.5">
                {SUMMARY_LENGTHS.map(({ id, label, desc }) => (
                  <button
                    key={id}
                    onClick={() => { if (!summaryLoading) { setSummaryLength(id); resetSummary() } }}
                    disabled={summaryLoading}
                    title={desc}
                    className={clsx(
                      'flex-1 text-[10px] py-1 rounded border transition-colors disabled:opacity-50',
                      summaryLength === id
                        ? 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 border-purple-300 dark:border-purple-700 font-semibold'
              ...
              {summaryDone && !summaryLoading && (
                <>
                  ...
                  <button
                    ...
                    className={clsx(
                      'ml-auto flex items-center gap-1 text-[10px] px-2.5 py-1 rounded border transition-colors',
                      noteSaved
                        ? 'text-green-600 dark:text-green-400 border-green-300 dark:border-green-700 bg-green-50 dark:bg-green-900/20'
                        : 'text-gray-500 dark:text-gray-400 border-gray-300 dark:border-gray-600 hover:text-gray-700 dark:hover:text-gray-200 hover:border-gray-400 dark:hover:border-gray-500'
                    )}
                  >
                    {noteSaved ? (
