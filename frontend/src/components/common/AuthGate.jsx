...
export default function AuthGate({ children }) {
  ...
  if (isLoading) {
    return (
      <div className="h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 text-gray-400 text-sm">
        <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm dark:border-white/10 dark:bg-slate-900">
          <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-blue-600 text-white shadow-sm">
            <BookOpen size={18} />
          </div>
          <p className="font-medium text-gray-700 dark:text-slate-200">Opening Scriptura</p>
          <p className="mt-1 text-xs text-gray-400 dark:text-slate-500">Checking your study space…</p>
          <div className="mx-auto mt-4 h-1 w-24 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
...
const cardCls =
  'bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-white/10 w-full max-w-sm'
