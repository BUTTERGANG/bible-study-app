...
# Configurable expiry (days). Override via SHARE_EXPIRY_DAYS env var.
...
class SharedSessionResolve(BaseModel):
    """Public response returned when resolving a share token."""
    book: str
    chapter: int
    translation: str
    passage: list[dict]  # [{verse, text}, ...]
    notes: list[dict]    # [{id, book, chapter, verse, content, tags}, ...]
    ai_conversation: dict | None  # {id, title, messages_summary}
    expires_at: datetime
    view_count: int
    created_at: datetime


# ── POST /api/shares — create share (requires auth) ─────────────────────
...
async def create_share(
    ...
    # Validate AI conversation belongs to user if specified
    ...
    # Validate note IDs belong to user
    if body.note_ids:
        ...
        if invalid:
            raise HTTPException(
                status_code=400,
                detail=f"Note IDs not found or not owned by user: {sorted(invalid)}",
            )
    ...
    shared = SharedSession(
        ...
        note_ids=json.dumps(body.note_ids),
    ...
    # Build the public URL — we use a relative path; the frontend will prepend origin
    url = f"/share/{token}"
...
# ── GET /api/shares/{token} — resolve share (PUBLIC, no auth) ───────────
...
async def resolve_share(
    ...
    result = await db.execute(
        select(SharedSession).where(SharedSession.share_token == token)
    )
    ...
    await db.execute(
        ...
        .where(SharedSession.id == shared.id)
        .values(view_count=SharedSession.view_count + 1)
    )
    await db.commit()

    # Fetch passage text (all verses in the chapter)
    ...
    passage = [
        ...
        for v in passage_q.scalars().all()
    ]
    ...
    notes: list[dict] = []
    if note_id_list:
        # We read notes owned by the sharing user — share is a snapshot
        notes_q = await db.execute(
            select(Note).where(Note.id.in_(note_id_list))
        )
        notes = [
            {
                "id": n.id,
    ...
    # Fetch AI conversation summary (if any)
    ai_conversation: dict | None = None
    if shared.ai_conversation_id is not None:
        conv_q = await db.execute(
            select(AiConversation).where(AiConversation.id == shared.ai_conversation_id)
        )
        conv = conv_q.scalar_one_or_none()
        if conv:
            # Parse messages and build a summary
            try:
            ...
            ai_conversation = {
                "id": conv.id,
                "title": conv.title or f"{conv.book} {conv.chapter}",
                "messages_summary": summary_messages,
            }
