...
async def get_verse_commentary(
    ...
    query = select(CommentaryEntry).where(
        CommentaryEntry.book == canonical,
        CommentaryEntry.chapter == chapter,
        CommentaryEntry.verse_start <= verse,
    ).filter(
        (CommentaryEntry.verse_end == None) | (CommentaryEntry.verse_end >= verse)
    ).order_by(CommentaryEntry.source)
    ...
    # Deduplicate: keep first entry per (source, verse_start, verse_end)
    seen = set()
    deduped = []
    for e in entries:
