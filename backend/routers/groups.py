...
def _note_dict(n: GroupNote, author_email: str = "", reply_count: int = 0) -> dict:
...
async def create_group_note(
    ...
    # If replying to a parent note, validate it exists and belongs to same group
    if body.parent_id is not None:
        parent_q = await db.execute(
            select(GroupNote).where(
                GroupNote.id == body.parent_id,
                GroupNote.group_id == group_id,
            )
        )
        parent = parent_q.scalar_one_or_none()
        if not parent:
            raise HTTPException(status_code=404, detail="Parent note not found in this group")
        # Replies go to the root parent (flatten to 1-level threading)
    ...
    # In-app notification: if this is a reply, notify the parent note's author
    if body.parent_id is not None:
        try:
            # Import here to avoid circular imports and allow graceful failure
            from ..models import InAppNotification  # noqa: F401
            parent_q2 = await db.execute(
                select(GroupNote).where(GroupNote.id == body.parent_id)
            )
            parent_note = parent_q2.scalar_one_or_none()
            if parent_note and parent_note.author_id != user.id:
                # Get group name for notification text
                grp_q = await db.execute(select(Group).where(Group.id == group_id))
                grp = grp_q.scalar_one_or_none()
                grp_name = grp.name if grp else "Group"
                notif = InAppNotification(
                    user_id=parent_note.author_id,
                    type="group_reply",
                    title=f"Reply in {grp_name}",
                    body=f"{email or 'Someone'} replied to your note",
                    data_json=f'{{"group_id": {group_id}, "note_id": {note.id}, "parent_id": {body.parent_id}}}',
                )
                db.add(notif)
                await db.commit()
        except (ImportError, Exception):
            # InAppNotification model may not exist yet; skip silently
            pass
...
async def list_group_notes(
    ...
    if threaded:
        # Return root notes (parent_id=NULL) with reply_count and nested replies
        root_q = (
        ...
        if chapter is not None:
            root_q = root_q.where(GroupNote.chapter == chapter)
        if verse is not None:
            root_q = root_q.where(GroupNote.verse == verse)
        if tag is not None:
            root_q = root_q.where(GroupNote.tags.ilike(f"%{tag}%"))
        root_q = root_q.order_by(GroupNote.created_at.desc()).offset(offset).limit(limit)

        root_result = await db.execute(root_q)
        root_rows = root_result.all()

        # Batch reply counts for all root notes
        root_ids = [n.id for n, _ in root_rows]
        reply_count_map: dict[int, int] = {}
        if root_ids:
            rc_q = await db.execute(
                select(GroupNote.parent_id, func.count())
                .where(GroupNote.parent_id.in_(root_ids))
                .group_by(GroupNote.parent_id)
            )
            reply_count_map = {pid: cnt for pid, cnt in rc_q.all()}

        # Batch-fetch all replies for root notes that have them
        replies_map: dict[int, list] = {}
        parent_ids_with_replies = [pid for pid, cnt in reply_count_map.items() if cnt > 0]
        if parent_ids_with_replies:
            replies_q = await db.execute(
                select(GroupNote, User.email)
                .join(User, GroupNote.author_id == User.id)
                .where(GroupNote.parent_id.in_(parent_ids_with_replies))
                .order_by(GroupNote.parent_id, GroupNote.created_at.asc())
            )
            for reply, reply_email in replies_q.all():
                replies_map.setdefault(reply.parent_id, []).append(
                    _note_dict(reply, author_email=reply_email)
                )

        notes = []
        for n, email in root_rows:
            d = _note_dict(n, author_email=email, reply_count=reply_count_map.get(n.id, 0))
            d["replies"] = replies_map.get(n.id, [])
            notes.append(d)

        return {"notes": notes, "offset": offset, "limit": limit}

    # Non-threaded: original flat listing
    query = (
        select(GroupNote, User.email)
        .join(User, GroupNote.author_id == User.id)
...
@router.get("/{group_id}/notes/{note_id}/replies")
async def get_note_replies(
    ...
    """Get all replies for a specific group note (thread)."""
    await _require_membership(db, group_id, user.id)
    ...
    # Fetch all replies ordered chronologically
    replies_q = await db.execute(
        select(GroupNote, User.email)
        .join(User, GroupNote.author_id == User.id)
        .where(GroupNote.parent_id == note_id)
        .order_by(GroupNote.created_at.asc())
    )

    return {
        "replies": [_note_dict(n, email) for n, email in replies_q.all()],
    }
...
async def get_group_feed(
    ...
    # Batch reply counts for group notes
    gn_ids = [gn.id for gn, _ in gn_rows]
    reply_count_map: dict[int, int] = {}
    if gn_ids:
        rc_q = await db.execute(
            select(GroupNote.parent_id, func.count())
            .where(GroupNote.parent_id.in_(gn_ids))
            .group_by(GroupNote.parent_id)
        )
        reply_count_map = {pid: cnt for pid, cnt in rc_q.all()}
    ...
    for gn, author_email in gn_rows:
        items.append({
            ...
            "content": gn.content,
            "tags": gn.tags,
            "reply_count": reply_count_map.get(gn.id, 0),
